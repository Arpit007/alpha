...
# Read Dataset
metadata = pd.read_csv('./dataset/ml-100k/u.data', sep = '\t', names = ['userId', 'itemId', 'rating', 'timestamp'], index_col = 0)
movieList = pd.read_csv('./dataset/ml-100k/u.item', sep = '|', header = None).rename({ 0: 'itemId', 1: 'title' },
                                                                                     axis = 'columns')
genreList = pd.read_csv('./dataset/ml-100k/u.genre', sep = '|', names = ['genre', 'id'])
...
def combineGenre(data):
	...
	genre = []
	for id, ele in enumerate(row.iloc[0, 6:]):
		if ele == 1:
			genre.append(id + 6)
	data['genre'] = genre
	return data
